...
def test_dashboard_includes_worker_health_widget() -> None:
    ...
    assert "href='/ui/assets'" in resp.text
    assert "href='/ui/collectors'" in resp.text
    assert "href='/ui/diagnostics'" in resp.text
...
def test_ui_diagnostics_page() -> None:
    ...
    assert "csv-export-link" in resp.text
    assert "refreshDiagnostics" in resp.text
